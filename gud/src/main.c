...
int main(int argc, char *argv[]){
  ...
  malla1D(-5.0,5.0,n,mc,nodos1);
  /* //Bloque de difusion conveccion 1D
  ConveccionDifusion1D(n,nodos1,2.5,0.1,1.0,0.0,sol);
  for(int i=0;i<n;++i)
    printf("%lf %lf\n",nodos1[i][0],sol[i]);
 /* */
  // ConveccionTransitoria1D(n,nodos1,0.01,5,1.0,0.0,inicial,sol);
   ConveccionTransitoria1D(n,nodos1,0.01,5,-1.0,0.0,inicial2,sol); //v<0
  // ConveccionTransitoriaImp1D(n,nodos1,0.01,5,1.0,0.0,inicial,sol);
  //ConveccionNolineal1D(n,nodos1,0.005,12,0.0,fburger,g0burger);
